...
### Fixed
- Slot fields added without an explicit `sortOrder` now append instead of landing at 0. The build page never sends one, so every field it added sorted ahead of template fields pinned at 1+ — which, with reminders anchored to the first date field, let a newly added empty date column take the anchor and null every `slot_at` on a live signup. `pnpm backfill:slot-at` pins the anchor on existing signups where the two rules disagree, before rebuilding.
