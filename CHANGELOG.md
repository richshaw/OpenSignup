...
### Security
- Next.js upgraded from 15.5.22 to 15.5.25, closing two critical advisories: unauthenticated remote code execution via the image optimization API when AVIF files are optimized ([GHSA-2xp9-vwfh-vxw4](https://github.com/advisories/GHSA-2xp9-vwfh-vxw4)), and unauthenticated remote code execution on Windows-hosted servers ([CVE-2026-75604](https://github.com/advisories/GHSA-p293-qw3h-jr36)). The first affects any deployment, the second matters to self-hosters on Windows.
- Nodemailer upgraded from 9.0.3 to 9.1.1, closing four advisories in the SMTP transport's address handling: a quadratic-time address parser that blocks the event loop ([GHSA-2x7j-588g-ccc2](https://github.com/advisories/GHSA-2x7j-588g-ccc2)), two recipient-domain parsing bugs that can deliver mail to an attacker-registrable domain ([GHSA-cc9r-2j5m-2m83](https://github.com/advisories/GHSA-cc9r-2j5m-2m83), [GHSA-wmmp-3585-3rmp](https://github.com/advisories/GHSA-wmmp-3585-3rmp)), and a `resolveContent()` sandbox bypass ([GHSA-8m3c-c648-2xjj](https://github.com/advisories/GHSA-8m3c-c648-2xjj)). `ParticipantEmailSchema` caps addresses at 254 characters and validates them before they reach the transport, so none of these were practically reachable from participant input; the bump removes the dependence on that.
