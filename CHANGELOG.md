...
- The public signup page's header is the instance name alone, linked to the homepage. The `· Public signup` segment beside it named nothing a participant could act on — they arrive from a shared link, and the title and Sign-up buttons already say what the page is — and "Public" contradicted the page's own `noindex`: these are unlisted share links, not listed pages.
...
- Every state of a slot row's action occupies the same box. "Full" and "Closed" were narrow labels hugging the row's edge next to an 82px "Sign up" pill, so one control's states did not line up with each other down the column. All of them now fill the design system's canonical 96px action column, contents centred (design-system/ui_kits/participant/SlotRow.jsx).
- The inert "Full"/"Closed" label is `text-sm`, matching the Sign-up button and Edit link it stands in for. It sat at `text-xs` — two sizes for one slot — while `text-ink-soft` was already carrying the de-emphasis on its own.
- Each slot row's action is named after its own slot (`Sign up for Wed, Sep 30`), not a bare `Sign up` repeated once per row, so a screen-reader user can tell which slot they are committing to.
- Row actions meet a 44px minimum touch target below `sm` (iOS HIG 44pt, Material 48dp). They measured 32px, in a list whose whole purpose is tapping one row out of many on a phone. Pointer widths keep the compact 32px pill.
